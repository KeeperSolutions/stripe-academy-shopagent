...
def checkout_success(session_id: str | None = None) -> HTMLResponse:
    ...
    try:
        ...
        return _render(
            ...
            "<p>This server has no Stripe key, so it cannot look the session "
            "up. Nothing was charged.</p>",
        )
    except Exception:
        ...
        return _render(
            "Checkout",
            "Session not found",
            "<p>That session id could not be read.</p>",
        )
...
<dl>
  ...
  <dt>Amount</dt><dd>{session.amount_total} {html.escape((session.currency or "").upper())}</dd>
